...
def plot_roc(y, y_, suptitle: str = None, display_n: bool = False, show_at_best_youden = False, text_bbox_alpha: float = .0, confidence_level: float = None):
    ...
    if show_at_best_youden:
        i_pos = np.argmax(tpr - fpr)
        plt.plot([fpr[i_pos], fpr[i_pos]], [0, 1], "-", color="black", lw=2, linestyle=':', alpha=.5)
        plt.plot([0, 1], [tpr[i_pos], tpr[i_pos]], "-", color="black", lw=2, linestyle=':', alpha=.5)
        plt.text(fpr[i_pos] + .02, tpr[i_pos] - .02,
                 f'Se={100*(1 - fpr[i_pos]):.1f}%, Sp={100*tpr[i_pos]:.1f}%',
                 verticalalignment="top",
                 horizontalalignment="left",
                 font=legend_font_params,
                 bbox=None)
